...
| Egg | 12 allocated-but-invisible eye nodes never surface | Unbuilt-as-designed, verified structural — `buildEgg` calls `makeEye` twice (`eye_left`/`eye_right`), each of which builds a 5-child container (shape/iris/pupil/2 catch-lights) at `alpha = 0`; 2 × 6 nodes (container + 5 children) = 12, matching the design's count exactly. Recommendation was to leave them skipped at Egg, same as shipped — flagged here only for completeness of the node inventory, not as a gap |
| Drop | Proto-ears made structural in the teardrop Bezier itself, not overlaid circles | Unbuilt — shipped Drop proto-ears are two separate `makeProtoEar` circle nodes at `alpha = 0.3` added as head children (`StageRenderer.swift`), not a modification to the teardrop body path. A triangular variant, `ShapeFactory.makeProtoEarTriangle`, exists in code but has zero call sites (grep-verified) — dead code adjacent to this recommendation, not the recommendation itself |
| Drop | Proto-tail shrunk to a 1-2pt nub at alpha 0.3 | Unbuilt — shipped proto-tail (`makeProtoTail(length: 3.0, alpha: 0.2)`) is larger and fainter than recommended |
| Drop | Eye radius 1.0pt -> 1.2pt | Unbuilt — shipped Drop eyes are `radius: 1.0` (`buildDrop`'s `eyeR` constant) |
| Critter | Head ratio `w × 0.3` -> `w × 0.35`, rounder circular paws | Unbuilt — shipped Critter head is `SKShapeNode(circleOfRadius: w * 0.3)`; shipped paws use the same `makePaw(size: 2)` shape as every other stage, no Critter-specific roundness variant |
| Apex | Pulsing-ring aura (expanding/contracting ring, distinct from the static alpha aura) | Unbuilt — same category as the Beast aura-pulse row above; Apex's aura pulsing was never carried into either stage's shipped `SKShapeNode` |
