...
| Tail counterbalance | Swing opposite the leading diagonal pair, gait-phase-locked | Feeds the shipped single-node `TailController` a new `gait_counterbalance` state, in the same style as its existing continuously-updated `sway` formula (`zRotation = baseRotation + amplitude × sin(...)`) — per [procedural animation's `TailController` section](/REFERENCE/procedural-animation.md#tailcontroller--shipped-the-tail-actually-running). **Not** the unbuilt `SegmentedTailController`'s target-angle-generator pattern (`computeTargets()`) — that spring-damper chain is [built but never instantiated](/REFERENCE/procedural-animation.md#segmentedtailcontroller--built-not-wired-force-based-spring-damper-reference), so a "target angle" reads here as new physics that doesn't ship; this is a new direct-formula state on the shipped controller, not new tail physics. |
