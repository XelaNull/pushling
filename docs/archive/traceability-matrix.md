...
- **371 source-section → concept mappings** (individual table rows) across
  ...
  fidelity proof.
- **57 items originally tagged `dropped-with-justification`** across all
  waves. SP8c reclassified one — MULTITOUCH §5 — from dropped to
  migrated-as-unbuilt before sign-off, leaving **56 approved as drops**.
  The Orchestrator signed off on those 56 of 57 on
  **2026-07-02T05:18:29Z** (`orchestrator.md`) — the section below is no
  longer awaiting a decision. The fidelity remediation pass then struck 2
  more (HUD overlay + evolution progress bar from P6-T1-02b; per-type
  invitation self-resolution from P6-T3-03), both found to be restorable
  unbuilt design intent rather than genuine drops — **54 drops stand
  today.** Most of the 54 are procedural (plan-wrapper scaffolding:
  Goal/Dependencies/QA-Gate headers, superseded MTMR/bash-era technique
  catalogs) rather than contested canon calls — the substantive ones are
  marked **⚑** below. All three reclassifications (§5, HUD/progress bar,
  P6-T3-03) are struck through with their overturn rationale in place,
  rather than deleted, so the audit trail of what was proposed-then-
  reversed stays visible.

# Intentional Drops — Orchestrator-Approved (56 of 57, 2026-07-02; 54 current)
...
- **P5-T1-12 "Apex World-Shaping Speech"** — did **not** land anywhere
  ...
  mechanism (`SpeechCoordinator.checkWorldShaping`, a 20-keyword trigger
  table (recounted this pass, correcting an earlier "18" miscount — see
  `worldShapeTriggers`, `SpeechCoordinator.swift:426-434`), a 5-minute
  cooldown, wired to `debugForceWeather` in
...
| Control Architecture: 4-Layer Behavior Stack, Blend Controller | `/SYSTEMS/behavior-stack.md` | migrated, verified + extended (sub-timing); the 12-item cat-behavior catalog (158–171) and absence-scaled wake/lantern behaviors (Core Loop, 393–404) inlined this pass as the Autonomous layer's own catalog, both fully shipped and code-verified. **Correction (RM-MOPUP):** the catalog's "name-for-name" claim was inaccurate — 10 of 12 names match the vision doc exactly, but the vision doc's "tail twitch" and "ear perk" entries correspond to no autonomous-behavior name in code; the code's 11th/12th slots are `tail_chase`/`tongue_blep` instead, two different behaviors. Corrected in-concept; substance of tail-twitch/ear-perk preserved as reflex/reactive-body-language cross-links | SP3a, RM-VISION, RM-MOPUP |
...
| The Commit-as-Food System | `/SYSTEMS/commit-feeding-xp.md` | migrated, corrected (17 commit types not 15; live award path bypasses `XPCalculator`); per-type *physical* choreography (442–460 — backward-eating reverts, both-sides merges, force-push knockback, food-coma, achievement popups) confirmed unbuilt and inlined as a labeled design-intent table (RM-VISION), plus the "tests are protein"/"docs are vegetables"/"styles are dessert"/"comfort food" framing lines identified as dead source-comment-only prose (RM-MOPUP). **Correction:** RM-VISION's claim of replacing a bare "full timings per VISION" pointer was itself overstated — a softened version of that pointer survived at the top of "The Eating Theater" section; **RM-MOPUP genuinely killed it this pass**, inlining the actual shipped per-character Feast-phase mechanics (0.1s shrink, 0.12s chewing bob, swallow-every-5th, the noodle-slurp text-slide — all code-verified live, not design intent) plus the still-unbuilt FEAST-MODE 5s-freeze/achievement-popup detail | SP7, RM-VISION, RM-MOPUP |
...
| Camera Controller (pan/zoom math) | `/SYSTEMS/camera-and-parallax.md` | migrated, corrected (R2: pan/zoom disabled behind a `FIXED-VIEWPORT` early return; documented as transitional, intent-canon preserved); **RM-TOUCH restored** the §4 two-finger-drag zoom fallback (200pt = 1.0 zoom level, left/right) as unbuilt intent into `/FEATURES/interactivity-unbuilt.md#live-pan--zoom` — SP5's "migrated, corrected" claim had dropped this input mapping entirely (no matrix drop row covered it). **RM-MOPUP** restored the §4 lock-trigger's **0.3s pan-zeroing rate** — code-verified as the *real* AutoLock mechanism (`pow(2, -dt/0.3)`, `CameraController.swift:270-277`), forced only at egg/drop stages, distinct from and faster than the general per-stage `decayHalfLife`; also corrected the source's claim that this rate fires on *tapping the creature to lock* at any stage — `lockToCreature()` has no pan-zeroing of its own. Also restored the §4 **pan/zoomLevel "consistent feel" rationale** (dividing dampened drag delta by `zoomLevel`) as unbuilt design intent — grep-confirmed `pan(deltaX:)` has no `zoomLevel` term at all |
...
| Tool Command Details (9 tools) | `/ARCHITECTURE/mcp-tool-contract.md`, `/ARCHITECTURE/ipc-command-catalog.md` | migrated, reconciled against code; **RM-ARCH restored** the full response-`data`-shape table for move/express/perform/world/teach/nurture into `ipc-command-catalog.md` — every shape code-verified and corrected against the archived doc's guesses (e.g. `move`'s real field is `position_x` not `position`; `world create`'s object has no `object_id`/`preset` fields; `world companion` has no `companion_id`; `teach compose` has no `draft_id`; `nurture habit` has no `habit_id`; `perform` has no `stage_ok`). **Correction (RM-MOPUP):** RM-ARCH's own `express` row was itself wrong — it claimed the daemon "only echoes `{expression, intensity, duration}` unchanged," but `ActionHandlers.swift:237-242` actually returns `{expression, intensity, duration_s, description}` (renamed field, plus a daemon-derived `description` the client doesn't end up using). Corrected in `ipc-command-catalog.md`. Also closed a stale cross-commit contradiction: `mcp-tool-contract.md`'s `sense`/`visual` row (436230c) still claimed no `FEATURES/` concept covered the screenshot design, even though `113969d` had already added exactly that section to `interactivity-unbuilt.md` — the two commits disagreed with each other; the stale flag is removed, now pointing directly at the P4-T1-06 section below |
...
| Track 2 (move/express/speak/perform/world specs) | `/ARCHITECTURE/mcp-tool-contract.md` | migrated, corrected (Drop speech limit 6 MCP / 3 daemon); **RM-ARCH restored** P4-T2-02's full 16-expression "Animation Description" table (HIGH finding) — code-verified **live and shipped**, word-for-word in `express.ts EXPRESSION_DESCRIPTIONS`, not unbuilt design intent. P4-T2-05 (companion behavior AI spec) **landed this pass (RM-MOPUP)** in `/SYSTEMS/world-objects-system.md#companions` — the shipped per-type `CompanionBehavior` vocabulary (`CompanionSystem.swift`, previously undocumented in any concept despite being fully shipped) is now enumerated with durations and the proximity-triggered reactive-behavior rule, code-verified against PHASE-4's lighter "wander/react/flee" summary (flee-from-touch confirmed unbuilt, react-to-creature confirmed shipped) | SP2a, RM-ARCH, RM-MOPUP |
...
| Section | → Target concept | Status | Wave |
|---|---|---|---|
| Track 1 (bubble rendering, Drop symbols, filtering, first-word ceremony, narration) | `/REFERENCE/speech-rendering.md`, `/SYSTEMS/speech-filtering.md`, `/REFERENCE/speech-milestones.md` | migrated, corrected throughout; RM-VOICE restored P5-T1-02's left-edge/both-edges bubble-position rules (found correct-by-default/physically-unreachable, not missing behavior), P5-T1-04/05's Drop-glyph animation numbers (0.2s fade-in and per-glyph emoji sizing confirmed NOT shipped; also found and corrected a stale doc claim that `♪` rotates — the `rotates` flag is dead code, nothing rotates), P5-T1-06's tokenization/emotion/scoring internals (word-tag hierarchy inversion at the bottom two tiers, missing commit-content scoring boost, missing 0.0–1.0 emotion confidence score — all code-verified), and P5-T1-13's Verbosity/Focus text-modifier rows (previously only Focus/Specialty were flagged missing; Verbosity was silently omitted from the loss note despite also being unbuilt) | SP4, RM-VOICE, RM-MOPUP |

**RM-MOPUP addendum:** the remaining P5-T1-04/05 gap (Drop floating-glyph 6pt upward-float distance and 2pt/1.5s sine-drift amplitude/period) is now restored in `speech-rendering.md`'s per-stage table — code-verified an exact match against `updateFloatingGlyph` (`SpeechBubbleNode.swift:490-501`), unlike the fade-in/emoji-sizing numbers above, which remain confirmed-not-shipped.
| P5-T1-08 Speech Cache & Replay | `/SYSTEMS/speech-filtering.md#speech-cache--replay-p5-t1-08` | **FIXED — landed for real this pass**, then **corrected further by RM-MOPUP after a deeper defect was found.** Restored in full under a dedicated section: cache storage (schema, `source`/`emotion`/`tts_cache_path` columns, 100-utterance FIFO eviction, `SpeechCache.swift:100-109`) plus all three designed replay consumers with their actual live/dead status, code-verified. **RM-MOPUP finding, supersedes "Dream replay is live" below:** `SpeechCache.createTableSQL`/`createIndexSQL` (the DDL that creates the `speech_cache` table itself) are never executed anywhere in the codebase — grep-verified zero call sites in `Migration.swift`, `DatabaseManager`, or `SpeechCache.init(db:)`. Every `store()`/query against `speech_cache` throws "no such table," caught and `NSLog`-ed, never crashing. **Dream replay is wired but non-functional** — `dreamUtterance()` is genuinely called from the wake-up path, but always returns `nil` since the table doesn't exist, so cached-utterance dream replay has never once fired in a real build. **Sage+ reminiscence is dead** (`failedSpeechEntries()`, zero callers, and would fail the same way if called). **Idle replay is dead** (`recentUtterances()`, zero callers, same failure mode) — unlike the other two, this scenario had no doc home anywhere in the bundle until RM-FIX-P5T108; documented fresh, distinguishing it from the Sage-reminiscence scenario it's easy to conflate with (no stage gate, no sleep requirement, full utterance not a fragment). Explicitly distinguished from the unrelated 50MB WAV audio-file cache at `voice-tts-stack.md#caching` (cross-linked, not duplicated), which is unaffected by this defect. Schema-level detail (DDL, the never-created-table finding) now also lives at [state-database-schema.md#speech_cache--designed-never-actually-created](/DATA_MODELS/state-database-schema.md#speech_cache--designed-never-actually-created), flagged for `DECISIONS.md`/the Orchestrator as a genuine functional bug, not a documentation gap. | SP4→SP8 (gap unresolved) →RM-VOICE (recorded, not landed) →RM-FINALIZE (drift caught, still open) →RM-FIX-P5T108 (landed) →RM-MOPUP (deeper runtime defect found and corrected) |
