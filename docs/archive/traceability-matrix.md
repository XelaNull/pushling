...
- **P5-T1-08 "Speech Cache & Replay"** — SP4 explicitly flagged this as an
  ...
  `failedSpeechEntries()` have zero callers). **Gap closed this pass**: see
  the PHASE-5.md per-source table row above for the full finding.
...
| P5-T1-08 Speech Cache & Replay | `/SYSTEMS/speech-filtering.md#failed-speech--the-journal`, `/SYSTEMS/voice-tts-stack.md#caching` | **gap found and filled this pass.** This section had no traceability-matrix row at all before RM-VOICE — SP4's own sub-matrix flagged it as an unresolved deferral ("if no other wave picks it up this is a gap to flag") and none did, through SP8's gap-closure pass. Code-verified finding: the cache is **not** unbuilt — `Pushling/Sources/Pushling/Speech/SpeechCache.swift` exists, is wired into `GameCoordinator`/`SpeechCoordinator`, and matches the 100-utterance FIFO cap, the `speech_cache` schema (style/source/emotion/`tts_cache_path`), and the dream-fragment logic exactly. What's genuinely unbuilt: the **idle-replay** consumer (`recentUtterances()` exists but has zero callers anywhere in `Speech/`) and the **Sage reminiscence** consumer (`failedSpeechEntries()` exists, also zero callers) — the cache's write/read API is complete, but neither of the two design-specified *replay scenarios* that consume it are wired to anything | SP4→SP8 (gap unresolved) →RM-VOICE (closed) |
