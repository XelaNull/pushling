...
| `visual` | Forwards to the `sense`/`visual` socket action — currently a not-yet-implemented acknowledgement, not a screenshot (see [the command catalog](/ARCHITECTURE/ipc-command-catalog.md)). **Not shipped, intent only:** the full designed JSON shape (natural-language scene description + inline `screenshot_base64` PNG at 2170×60 @2x, <50ms capture/<100ms added latency), contrasted against both the current bare ack and the separate operator-only `screenshot` command's path-returning shape, is documented in full at [Visual Sense — Full Screenshot Design Intent](/FEATURES/interactivity-unbuilt.md#visual-sense--full-screenshot-design-intent-p4-t1-06) — not duplicated here. |
